...
class Log:
    ...
    def make(self, name, attributes):
        return self._make(name, attributes)
    ...
    def _get_name(self):
        if self.content["name"]:
            return JSON + "/" + str(int(self.content["name"])-1).rjust(4, "0") + ".json"
        return JSON + "/" + "test.json"

    def _make(self, name, attributes):
        self.reset()
        self.content["name"] = f"Cryptopanties #{name}"
    ...
    def produce(self):
        ...
        for name, attr in self._attributes.items():
            if name.lower() not in ["stitch"]:
                _attribute_collection.append(attr(name, self._db))
...
class LogAttribute:
    def __init__(self, name="generic", db=None) -> None:
        self.name = name
        self.value = None
        self._db = db
...
class NftLogAttribute(LogAttribute):
    def __init__(self, name="Upcycled NFT", db=None) -> None:
...
class NftAuthorLogAttribute(LogAttribute):
    def __init__(self, name="NFT Author", db=None) -> None:
        LogAttribute.__init__(self, name, db=db)
...
class PantiesModelLogAttribute(LogAttribute):
    def __init__(self, name="Identity", db=None) -> None:
        LogAttribute.__init__(self, name, db=db)
...
class PartNumberLogAttribute(LogAttribute):
    def __init__(self, name="Parts", db=None) -> None:
        LogAttribute.__init__(self, name, db=db)
...
class FabricLogAttribute(LogAttribute):
    def __init__(self, name="Fabric", db=None) -> None:
        LogAttribute.__init__(self, name, db=db)
...
class TrimLogAttribute(LogAttribute):
    def __init__(self, name="Trim", db=None) -> None:
        LogAttribute.__init__(self, name, db=db)
